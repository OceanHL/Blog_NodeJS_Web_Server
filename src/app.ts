...
import { handleBlogRouter } from './router';
import { handleUserRouter } from './router';
import { getSession, setSession } from './db/redis';
import { writeAccessLog } from './utils/log';
 ...
 * @param req
 */
function getPostData<T = { [key: string]: any }>(req: MyIncomingMessage): Promise<T> {
    const promise = new Promise<T>(resolve => {
        ...
        req.on('end', () => {
            if (!postData) return resolve({} as T);
...
const serverHandle: MyRequestListener<void> = (req, res) => {
    // 记录 access log
    ...
    cookieStr.split(';').forEach(item => {
        ...
        if (req.cookie) req.cookie[key] = value;
    });
